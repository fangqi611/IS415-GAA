...
In this section I will install and load my tidyverse and sf packages.

```{r}
pacman::p_load(tidyverse, sf)
```

# 1.1 Importing a geospatial data in shapefile format
...
## 1.1.1 Plotting the Geospatial Data
...
## 1.1.2 Importing GIS data in kml format
...
able above shows the content of `listing_sf`. Notice that a new column called `geometry` has been added into the data frame. On the other hand, the `longitude` and `latitude` columns have been dropped from the data frame.
...
buffer_cycling <- st_buffer(cyclingpath, 
                               dist=5, nQuadSegs = 30) #the higher the buffer (QuadSegs) the smoother it is
```
